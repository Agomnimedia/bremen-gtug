...
public class SingleGameActivity extends SimpleBaseGameActivity implements
	...
	protected void onCreate(Bundle pSavedInstanceState) {
		...
		if (Build.VERSION.SDK_INT > 10) {
			this.mRenderSurfaceView
					.setSystemUiVisibility(View.SYSTEM_UI_FLAG_LOW_PROFILE);
		}

		this.mode = this.getIntent().getExtras()
				.getInt(MultiPlayerLabyrinthActivity.MODE_EXTRA_NAME);
	}
	...
	protected Scene onCreateScene() {
		...
		this.levelCreator = new LevelCreatorImpl(
				this.getVertexBufferObjectManager(), this.theme, null);
	...
	public void onAccelerationChanged(AccelerationData pAccelerationData) {
		if (this.levelCreator != null) {
			final Vector2 gravity = Vector2Pool.obtain(
					pAccelerationData.getX() * 10,
					pAccelerationData.getY() * 10);
