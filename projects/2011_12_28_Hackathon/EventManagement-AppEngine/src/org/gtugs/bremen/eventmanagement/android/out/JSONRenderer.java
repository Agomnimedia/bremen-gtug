	...
	public String renderEvent(Event event, DataAmount amount) {
		...
		try{
			...
			case COMPLETE:
				...
				for(Attendee att : event.getAttendies()){
					array.put(new JSONObject(this.renderAttendee(att, DataAmount.LIGHT)));
				}
				json.put("attendees", array);
				json.put("admin", new JSONObject(this.renderAttendee(event.getAdmin(), DataAmount.LIGHT)));
				json.put("talks", event.getTalks());
				json.put("maxAttendies", event.getMaxAttendees());
				final JSONArray arrayTalks = new JSONArray();
				for(Talk talk : event.getTalks()){
					arrayTalks.put(new JSONObject(this.renderTalk(talk, DataAmount.LIGHT)));
				}
				break;
			case LIGHT:
				json.put("key", KeyFactory.keyToString(event.getKey()));
	...
	public String renderAttendee(Attendee attendee, DataAmount amount) {
		...
		try{
			...
			case COMPLETE:
				...
				for(Event ev : attendee.getEvents()){
					jsonArray.put(new JSONObject(this.renderEvent(ev, DataAmount.LIGHT)));
				}
