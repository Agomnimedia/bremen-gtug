...
/** 
 * DAO for table ATTENDEE_ENTITY (schema version 1).
*/
public class AttendeeEntityDao extends AbstractDao<AttendeeEntity, Long> {
    ...
    public static void createTable(SQLiteDatabase db, boolean ifNotExists) {
        String sql = "CREATE TABLE " + (ifNotExists? "IF NOT EXISTS ": "") + "'ATTENDEE_ENTITY' (" + //
    ...
    @Override
    protected void attachEntity(AttendeeEntity entity) {
    ...
    protected String getSelectDeep() {
        if (selectDeep == null) {
            ...
            builder.append(',');
            SqlUtils.appendColumns(builder, "T0", daoSession.getEventEntityDao().getAllColumns());
            builder.append(" FROM ATTENDEE_ENTITY T");
            builder.append(" LEFT JOIN EVENT_ENTITY T0 ON T.'EVENT_ID'=T0.'_id'");
            builder.append(' ');
    ...
    protected AttendeeEntity loadCurrentDeep(Cursor cursor, boolean lock) {
        AttendeeEntity entity = loadCurrent(cursor, 0, lock);
        int offset = getAllColumns().length;

        EventEntity eventEntity = loadCurrentOther(daoSession.getEventEntityDao(), cursor, offset);
        entity.setEventEntity(eventEntity);

        return entity;    
    }

    public AttendeeEntity loadDeep(Long key) {
        assertSinglePk();
        if (key == null) {
            return null;
        }
    ...
    public List<AttendeeEntity> loadAllDeepFromCursor(Cursor cursor) {
        int count = cursor.getCount();
        List<AttendeeEntity> list = new ArrayList<AttendeeEntity>(count);
    ...
    protected List<AttendeeEntity> loadDeepAllAndCloseCursor(Cursor cursor) {
        try {
    ...
    public List<AttendeeEntity> queryDeep(String where, String... selectionArg) {
