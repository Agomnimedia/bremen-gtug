...
function buildNextItem() {
  ...
  var toHightlight  = slideEls[curSlide].querySelectorAll('.to-hightlight');

  if (!toBuild.length && !toHightlight.length) {
    return false;
  }
  ...
  toHightlight[0].classList.remove('to-hightlight', '');
  toHightlight[0].classList.add('hightlight');
...
function addSliteFooters() {
	if (document.getElementById("footerLeft") || document.getElementById("footerRight") || document.getElementById("copyright")) {
		...
		var slides = document.querySelectorAll('section.slides > article');
		var last_h2 = "";
		for (var i = 0; i < slides.length; i++) {
			var h2 = slides[i].querySelectorAll('h2');
			if(h2.length != 0) {
			...
			if (document.getElementById("copyright")) {
				var cr = document.getElementById("copyright").cloneNode(true);
				cr.innerHTML = replaceFields(cr.innerHTML, i + 1, last_h2);
				cr.id = null;
				slides[i].appendChild(cr);
			}
		}
	}
}
